function y = least_squares_overlap_add(x, N, Nw, Ns)
% OVERLAP_ADD - reconstruct sequence using overlap add.
%
% Inputs:
%	x - framed synthesised sequence.
...
%% BRIEF:          Reconstruct synthesized sequence from frames using overlap-add method.
    ...
    for j = 1:M
        y(i(j,:)) = y(i(j,:)) + (x(j,:).*w); % overlap-add processed frames.
        w_sum(i(j,:)) = w_sum(i(j,:)) + (w.^2); % overlap-add window samples.
    end
